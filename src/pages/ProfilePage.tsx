...
const ProfilePage = () => {
  ...
  const [profileData, setProfileData] = useState({
    // Personal Information - use real user data
    ...
    // Citizen Rewards Information - use real user data
    membershipLevel: user?.membershipLevel || 'Bronze',
    occupation: user?.occupation || '',
    employer: '',
    citizenId: `CR-2024-${user?.id || '000000'}`,
  ...
  // Stats for citizen engagement - use real user data
  const userStats = {
    totalPoints: user?.totalPoints || 0,
    redeemedPoints: user?.redeemedPoints || 0,
    memberSince: user?.memberSince || 'October 2025',
    currentStreak: user?.currentStreak || 0,
    activitiesCompleted: user?.activitiesCompleted || 0,
    communityImpact: user?.communityImpact || 'New Member',
